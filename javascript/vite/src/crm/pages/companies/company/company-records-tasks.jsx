...
const todayTasks = [
  ...
  },
  {
    ...
    org: 'KeenThemes',
    assignees: [
      { name: 'Cody Fisher', avatar: '/media/avatars/300-9.png' },
      { name: 'Robert Anderson', avatar: '/media/avatars/300-8.png' },
    ],
  },
];
...
const yesterdayTasks = [
  ...
  },
  {
    ...
    org: 'KeenThemes',
    assignees: [{ name: 'Cody Fisher' }, { name: 'Robert Anderson' }],
  },
];
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <span className="inline-flex items-center gap-1 cursor-pointer">
            <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold text-white bg-gray-400">
              {assignees.length}
            </span>
        ...
        <TooltipContent className="p-2 flex gap-3.5">
          {assignees.map((a) => (
            <div key={a.name} className="flex items-center gap-2">
              <Avatar className="size-4">
                <AvatarImage
                  src={toAbsoluteUrl(getUniqueAvatar(a.name).avatar)}
                  alt={a.name}
                />
...
export function CompanyRecordsTasks() {
  ...
  const [yesterday, setYesterday] = useState(yesterdayTasks);
  const [lastWeek, setLastWeek] = useState(lastWeekTasks);

  const handleDelete = (group, id) => {
    if (group === 'today')
      setToday((tasks) => tasks.filter((t) => t.id !== id));
    if (group === 'yesterday')
      setYesterday((tasks) => tasks.filter((t) => t.id !== id));
    if (group === 'lastWeek')
      setLastWeek((tasks) => tasks.filter((t) => t.id !== id));
  };
