...
const Inventory = ({}: InventoryProps) => {
  ...
  const renderRow = (row: InventoryRow, index: number) => {
    return (
      <div key={index} className="flex items-center justify-between bg-gray-50 rounded-md px-4 py-2 text-sm gap-2">
        <span className="text-gray-900 font-normal">{row.name}</span>
        <div className="flex items-center gap-3">
          <span className="text-gray-900 font-normal shrink-0">Qty: {row.qty}</span>
          <Separator className='bg-gray-300 h-3' orientation="vertical" />
          <Link to="#" className="hover:text-primary hover:underline hover:underline-offset-2">
            Order
          </Link>  
        </div>
      </div>
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent className="flex flex-col gap-2 p-5 lg:p-7.5">
        ...
        <div className="flex items-center justify-between mb-0.5">
          <span className="text-dark font-medium text-sm">Low stock</span>
          <Button mode="link" underline="solid" asChild>
            <Link to="#">See All</Link>
          </Button>
        </div>
