...
export function NoteList({ filter }: NoteListProps) {
    ...
    return notes.filter((note) => {
      switch (filter) {
        case 'today':
          ...
          const noteDate = new Date(noteDueDate.getFullYear(), noteDueDate.getMonth(), noteDueDate.getDate());
          return noteDate.getTime() === today.getTime() && (note.status !== 'completed' || recentlyCompleted.has(note.id));
        case 'week':
          return new Date(note.dueAt) <= weekFromNow && (note.status !== 'completed' || recentlyCompleted.has(note.id));
        case 'completed':
  ...
  const filteredData = useMemo(() => {
    return filteredByTab.filter((note) => {
      const matchesStatus = !selectedStatuses.length || selectedStatuses.includes(note.status || 'pending');
      const matchesPriority = !selectedPriorities.length || selectedPriorities.includes(note.priority);
      const matchesCategory = !selectedCategories.length || selectedCategories.some((category) => note.category.includes(category));
  ...
  }, [filteredByTab, searchQuery, selectedStatuses, selectedPriorities, selectedCategories]);
  ...
  const handleStarClick = (noteId: string) => {
    ...
    if (mockNoteIndex !== -1) {
      mockNotes[mockNoteIndex] = { ...mockNotes[mockNoteIndex], isFavorite: !mockNotes[mockNoteIndex].isFavorite };
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge variant="success" appearance="light">Completed</Badge>;
      case 'in_progress':
        return <Badge variant="info" appearance="light">In Progress</Badge>;
      case 'pending':
      default:
  ...
  const getPriorityBadge = (priority: string) => {
    switch (priority) {
      case 'high':
        return <Badge variant="destructive" appearance="light">High</Badge>;
      case 'medium':
        return <Badge variant="warning" appearance="light">Medium</Badge>;
      case 'low':
      default:
    ...
    () => [ 
      {
        ...
        id: 'title',
        header: ({ column }) => (
          <DataGridColumnHeader title="Note" visibility={true} column={column} className="-ms-px" />
        ),
        cell: ({ row }) => {
      ...
      },
      {
        ...
        id: 'category',
        header: ({ column }) => (
          <DataGridColumnHeader title="Category" visibility={false} column={column} />
        ),
        cell: ({ row }) => (
          <div className="flex gap-1 flex-wrap">
            {row.original.category.map((category) => (
              <Badge
                key={category}
                variant={
                  category === 'client' ? 'primary' :
                  category === 'daily' ? 'info' :
                  category === 'team' ? 'warning' :
                  category === 'demo' ? 'success' :
                  category === 'presentation' ? 'destructive' :
                  category === 'code' ? 'secondary' :
                  category === 'review' ? 'warning' :
                  category === 'maintenance' ? 'destructive' :
                  category === 'backup' ? 'info' :
                  category === 'report' ? 'secondary' :
                  category === 'weekly' ? 'success' :
                  category === 'prospect' ? 'info' :
                  category === 'sales' ? 'primary' :
                  category === 'crm' ? 'warning' :
                  category === 'proposal' ? 'destructive' :
                  category === 'urgent' ? 'destructive' :
                  'success'
                }
                appearance="light"
              >
      ...
      }, 
      {
        ...
        id: 'team',
        header: ({ column }) => (
          <DataGridColumnHeader title="Assigned" visibility={true} column={column} />
        ),
        cell: ({ row }) => {
          const contactIds = row.original.assignedContactIds || [];
          return (
            <div className="flex truncate overflow-hidden gap-1.5">
              {contactIds.slice(0, 3).map((contactId: Key | null | undefined) => {
              ...
              {contactIds.length > 3 && (
                <Badge variant="secondary" appearance="light">+{contactIds.length - 3}</Badge>
              )}
            </div>
          );
        },
      ...
      },
      {
        ...
        id: 'priority',
        header: ({ column }) => (
          <DataGridColumnHeader title="Priority" visibility={true} column={column} />
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const note = row.original;
          const isOverdue = new Date(note.dueAt) < new Date() && note.status !== 'completed';
          return (
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-2">
            ...
            >
              <Star className={`text-gray-200 ${row.original.isFavorite ? 'text-yellow-500 fill-yellow-500' : 'fill-gray-200'}`} />
            </Button>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" side="bottom">
                <DropdownMenuItem variant="destructive" onClick={() => handleDeleteNote(row.original.id)}>
                  <Trash />
                  Delete
                </DropdownMenuItem>
              </DropdownMenuContent>
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <Badge
                                  variant={
                                    status.id === 'completed' ? 'success' :
                                    status.id === 'in_progress' ? 'info' :
                                    'secondary'
                                  }
                                  appearance="light"
                          ...
                          return (
                            ...
                            >
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => handleCategoryChange(checked === true, category.id)}
                                size="sm"
                              />
                              <Label
                                htmlFor={category.id}
                              ...
                              >
                                <Badge
                                  variant={
                                    category.id === 'client' ? 'primary' :
                                    category.id === 'daily' ? 'info' :
                                    category.id === 'team' ? 'warning' :
                                    category.id === 'demo' ? 'success' :
                                    category.id === 'presentation' ? 'destructive' :
                                    category.id === 'code' ? 'secondary' :
                                    category.id === 'review' ? 'warning' :
                                    category.id === 'maintenance' ? 'destructive' :
                                    category.id === 'backup' ? 'info' :
                                    category.id === 'report' ? 'secondary' :
                                    category.id === 'weekly' ? 'success' :
                                    category.id === 'prospect' ? 'info' :
                                    category.id === 'sales' ? 'primary' :
                                    category.id === 'crm' ? 'warning' :
                                    category.id === 'proposal' ? 'destructive' :
                                    category.id === 'urgent' ? 'destructive' :
                                    'success'
                                  }
                                  appearance="light"
                                  className="px-1.5 py-0.5 text-xs"
