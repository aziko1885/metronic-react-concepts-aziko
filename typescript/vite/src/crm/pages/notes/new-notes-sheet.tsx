...
const EMPLOYEE_RANGES = [
  { id: 'pending', name: 'Pending', color: 'bg-primary' },
  { id: 'in_progress', name: 'In Progress', color: 'bg-info' },
      ...
      <SheetContent className="sm:w-[600px] sm:max-w-none inset-5 start-auto h-auto rounded-lg p-0 [&_[data-slot=sheet-close]]:top-4.5 [&_[data-slot=sheet-close]]:end-5">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea rows={6} placeholder="Enter note details..." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                            ...
                            >
                              {field.value ? (
                                <span className="flex items-center gap-2.5">
                                  <span className={cn('ms-0.5 size-1.5 rounded-full', priorityOptions.find(p => p.value === field.value)?.state)}></span>
                                  <span className="truncate">{priorityOptions.find(p => p.value === field.value)?.label}</span>
                                </span>
                              ) : (
                              ...
                              <CommandList>
                                ...
                                <CommandGroup>
                                  {priorityOptions.map((priority) => (
                                    ...
                                    >
                                      <span className="flex items-center gap-2.5">
                                        <span className={cn('ms-0.5 size-1.5 rounded-full', priority.state)}></span>
                        ...
                        <Popover open={categoryOpen} onOpenChange={setCategoryOpen}>
                          <PopoverTrigger asChild>
                            ...
                            >
                              {field.value ? (
                                <span className="flex items-center gap-2.5">
                                  <span className={cn('ms-0.5 size-1.5 rounded-full', categoryOptions.find(c => c.value === field.value)?.state)}></span>
                                  <span className="truncate">{categoryOptions.find(c => c.value === field.value)?.label}</span>
                                </span>
                              ) : (
                              ...
                              <CommandList>
                                ...
                                <CommandGroup>
                                  {categoryOptions.map((category) => (
                                    ...
                                    >
                                      <span className="flex items-center gap-2.5">
                                        <span className={cn('ms-1 size-1.5 rounded-full', category.state)}></span>
                                        <span className="truncate">{category.label}</span>
                                      </span>
                                      {field.value === category.value && <CommandCheck />}
                                    </CommandItem>
                                  ))}
                                </CommandGroup>
                              </CommandList>
        ...
        <SheetFooter className="flex items-center not-only-of-type:justify-between border-t py-3.5 px-5 border-border">
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="create-more" className="text-xs text-secondary-foreground">Create more notes</Label>
          </div>
