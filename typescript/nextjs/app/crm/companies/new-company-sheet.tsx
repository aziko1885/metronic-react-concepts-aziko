...
import { CONNECTION_STRENGTHS } from '@/app/crm/mock/companies';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const FormSchema = z.object({
  ...
  domain: z.string().url('Please enter a valid domain URL').optional().or(z.literal('')),
  ...
  angelList: z.string().url('Please enter a valid URL').optional().or(z.literal('')),
  linkedin: z.string().url('Please enter a valid LinkedIn URL').optional().or(z.literal('')),
  connectionStrengthId: z.string().min(1, 'Please select a connection strength'),
...
export function NewCompanySheet({
  ...
  const [availabilityTimeSlots, setAvailabilityTimeSlots] = useState<{ time: string; available: boolean }[]>([]);
  ...
  const onError = (errors: FieldErrors<z.infer<typeof FormSchema>>) => {
    ...
    toast.custom((t) => (
      <Alert variant="mono" icon="destructive" onClose={() => toast.dismiss(t)}>
        ...
        <AlertTitle>Please fix the validation errors before submitting</AlertTitle>
      </Alert>
    ), {
      position: 'top-center'
    });
      ...
      <SheetContent className="sm:w-[600px] sm:max-w-none inset-5 start-auto h-auto rounded-lg p-0 [&_[data-slot=sheet-close]]:top-4.5 [&_[data-slot=sheet-close]]:end-5">
        ...
        <SheetBody className="p-0">
          <ScrollArea className="h-[calc(100dvh-11.75rem)] ps-3 pe-2 me-1">
            <TooltipProvider>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit, onError)} className="space-y-6 px-2">
                  <FormField
                        ...
                        <FormItem>
                          <div className="flex items-center gap-2">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>Enter a brief description of the company (minimum 10 characters)</p>
                              </TooltipContent>
                            </Tooltip>
                          </div>
                          <FormControl>
                        ...
                        <FormItem>
                          <div className="flex items-center gap-2">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>Select the strength of your connection with this company</p>
                              </TooltipContent>
                            </Tooltip>
                          </div>
                          <FormControl>
                            <Popover open={connectionOpen} onOpenChange={setConnectionOpen}>
                                ...
                                >
                                  {field.value ? (
                                    <div className="inline-flex items-center gap-1.5">
                                      <span className={cn('rounded-full size-2', connectionStrengthOptions.find(c => c.value === field.value)?.color)}></span>
                                      <span className={cn('text-medium text-foreground')}>
                                        {connectionStrengthOptions.find(c => c.value === field.value)?.label}
                                      </span>
                                    </div>
                                  ) : (
                              ...
                              <PopoverContent className="w-[200px] p-0" align="start">
                                <Command>
                                  <CommandInput placeholder={field.value ? undefined : "Select connection strength..."} />
                                  <CommandList>
                                    <CommandEmpty>No connection strength found.</CommandEmpty>
                                    <CommandGroup>
                                      {connectionStrengthOptions.map((strength) => (
                                        <CommandItem
                                          key={strength.value}
                                        ...
                                        >
                                          <div className="inline-flex items-center gap-1.5">
                                            <span className={cn('rounded-full size-2', strength.color)}></span>
                                            <span className={cn('text-medium text-foreground')}>
                                              {strength.label}
                                            </span>
                                          </div>
                                          {field.value === strength.value && <CommandCheck />}
                                        </CommandItem>
                                      ))}
                                    </CommandGroup>
                                  </CommandList>
                            ...
                            <Popover open={locationOpen} onOpenChange={setLocationOpen}>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  {field.value ? (
                                    <div className="inline-flex items-center gap-1.5">
                                      <span>{companies.find(c => c.id === field.value)?.country},</span>
                                      <span>{companies.find(c => c.id === field.value)?.city}</span>
                                    </div>
                                  ) : (
                                    ...
                                    <CommandGroup>
                                      {companies.map((company) => (
                                        ...
                                        >
                                          ...
                                          {field.value === company.id && <CommandCheck />}
                                        </CommandItem>
                                      ))}
                                    </CommandGroup>
                                  </CommandList>
                        ...
                        <FormItem>
                          <div className="flex items-center gap-2">
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent>
                                <p>Select the primary business category for this company</p>
                              </TooltipContent>
                            </Tooltip>
                          </div>
                          <FormControl>
                                ...
                                >
                                  {field.value ? (
                                    <span className="inline-flex items-center gap-2.5">
                                      <span className={cn('size-2 rounded-full', CATEGORIES.find(c => c.id === companies.find(c => c.id === field.value)?.categoryIds?.[0])?.bullet)}></span>
                                      <span className="truncate">{CATEGORIES.find(c => c.id === companies.find(c => c.id === field.value)?.categoryIds?.[0])?.name}</span>
                                    </span>
                                  ) : (
                                  ...
                                  <CommandList>
                                    ...
                                    <CommandGroup>
                                      {CATEGORIES.map((category) => (
                                        ...
                                        >
                                          <div className="inline-flex items-center gap-1.5">
                                            <span className={cn('rounded-full size-2', category.bullet)}></span>
                                            <span className={cn('text-medium text-foreground')}>
                                              {category.name}
                                            </span>
                                          </div>
                                          {field.value === category.id && <CommandCheck />}
                                        </CommandItem>
                                      ))}
                                    </CommandGroup>
                                  </CommandList>
                                ...
                                >
                                  {field.value ? (
                                    <span className="inline-flex items-center gap-1.5">
                                      <span className={cn('rounded-full size-2', ESTIMATED_ARRS.find(a => a.id === field.value)?.color.replace('50', '500'))}></span>
                                      <span className={cn('text-medium text-foreground')}>
                                        {ESTIMATED_ARRS.find(a => a.id === field.value)?.name}
                                      </span>
                                    </span>
                                  ) : (
                                    <span className="text-muted-foreground">Select ARR</span>
                                  )}
                                  <ButtonArrow />
                                </Button>
                              </PopoverTrigger>
                                  ...
                                  <CommandList>
                                    <CommandGroup>
                                      {ESTIMATED_ARRS.map((arr) => (
                                        ...
                                        >
                                          <span className="inline-flex items-center gap-1.5">
                                            <span className={cn('rounded-full size-2', arr.color.replace('50', '500'))}></span>
                                            <span className={cn('text-medium text-foreground')}>
                                              {arr.name}
                                            </span>
                                          </span>                                  
                            ...
                            <Popover open={employeeOpen} onOpenChange={setEmployeeOpen}>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  {field.value ? (
                                    <span className="inline-flex items-center gap-1.5">
                                      <span className={cn('rounded-full size-2', EMPLOYEE_RANGES.find(r => r.id === field.value)?.color.replace('50', '500'))}></span>
                                      <span className={cn('text-medium text-foreground')}>
                                        {EMPLOYEE_RANGES.find(r => r.id === field.value)?.name}
                                      </span>
                                    </span>
                                  ) : (
                                    <span className="text-muted-foreground">Select Employee Range</span>
                                  )}
                                  <ButtonArrow />
                                </Button>
                              </PopoverTrigger>
                                        ...
                                        >
                                          <span className="inline-flex items-center gap-1.5">
                                            <span className={cn('rounded-full size-2', 
                                              range.color.includes('stone') ? 'bg-stone-500' :
                                              range.color.includes('red') ? 'bg-red-500' :
                                              range.color.includes('orange') ? 'bg-orange-500' :
                                              range.color.includes('yellow') ? 'bg-yellow-500' :
                                              range.color.includes('lime') ? 'bg-lime-500' :
                                              range.color.includes('green') ? 'bg-green-500' :
                                              range.color.includes('sky') ? 'bg-sky-500' :
