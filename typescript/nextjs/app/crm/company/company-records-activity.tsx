...
import { CalendarRange, CalendarClock, SquareDashedBottomCode } from 'lucide-react';
      ...
      items: [
        {
          ...
          noteContent: 'Updated sales strategy for Q2 focusing on enterprise clients.',
          org: 'KeenThemes',
          time: '13 days ago',
        },
      ],
  ...
  'May 2025': [
    {
      ...
      items: [
        ...
        },
        {
          ...
          noteContent: 'Created comprehensive onboarding checklist for new clients.Updated marketing strategy for Q3 campaigns.',
      ...
      {Object.entries(activitiesByMonth).map(([month, activityGroups]) => (
        <div key={month} className="relative mb-10">
          <button
            ...
            onClick={() => setExpanded((prev) => ({ ...prev, [month]: !prev[month] }))}
          ...
          >
            ...
            <span className={`transition-transform ${expanded[month] ? '' : '-rotate-90'}`}>
                ...
                return (
                  <div key={group.date} className="space-y-5">
                    {group.items.map((item, itemIdx) => {
                      const isLastGroup = groupIdx === activityGroups.length - 1;
                      const isLastItem = itemIdx === group.items.length - 1;
                      const isLast = isLastGroup && isLastItem;

                      return (
                        <div key={`${group.date}-${itemIdx}`} className="relative flex gap-3 mb-5 last:mb-0">
                            ...
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2 flex-wrap relative">
                                ...
                                {item.target && item.targetUrl && (
                                  <Link href={item.targetUrl} className="hover:underline text-primary text-sm">
                                    {item.target}
                                  </Link>
                                )}
                                {item.noteTitle && <span className="font-semibold text-sm">{item.noteTitle}</span>}
                                {item.org && (
                                  <span className="ml-2 bg-muted px-2 py-0.5 rounded text-xs">{item.org}</span>
                                )}
                              </div>

                              {item.noteContent && (
                                <div className="ml-8 text-xs text-muted-foreground mt-0.5">{item.noteContent}</div>
                              )}
                            </div>
                    ...
                    {month === 'June 2025' && groupIdx === activityGroups.length - 1 && (
                        ...
                        <div className="flex gap-2">
                          ...
                          <div className="flex flex-col gap-3 grow">
                            <div className="flex flex-wrap items-center justify-between">
                              ...
                              <Button className='-mt-6' mode="link" underlined="dashed">
                                <Link href="#">View</Link>
                              </Button>
                            </div>
                    ...
                    {month === 'May 2025' && groupIdx === activityGroups.length - 1 && (
                          ...
                          <div className="flex flex-col md:flex-row md:items-center gap-2">
                            <div className="flex items-center gap-2 shrink-0">
                              <div className="border border-orange-200 rounded-lg max-h-20">
                                ...
                                <div className="flex items-center justify-center size-12">
                                  <span className="font-medium text-foreground text-sm tracking-tight">02</span>
                                </div>
                              </div>
                              <img
                            ...
                            <div className="flex flex-col items-start gap-2">
                              ...
                              <p className="text-xs text-foreground leading-[22px]">
                                Expert-led workshop. Learn advanced techniques and capture stunning scenes.
                              </p>
                            </div>
                          </div>
                        </div>
